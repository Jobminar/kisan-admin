...
const FreshVegitables = () => {
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      ...
      const apiUrl = `http://localhost:4000/get-vegetables`;
  ...
  const handleItemDelete = async (itemId) => {
    ...
    try {
      const deleteUrl = `http://localhost:4000/deleteVegetable/${itemId}`; // Use the item ID in the delete URL
      ...
      {inventoryData.map((item) => (
        <div key={item._id} className="product-sub-con">
          ...
          <div className="edit-delete-buttons">
            ...
            <Button onClick={() => openPopup("freshVegitables", item._id)}>
              <CreateOutlinedIcon />
              Edit Vegitables
            </Button>
          </div>
...
export default FreshVegitables;
